...
export default function Game2048() {
  ...
  const move = (direction: 'UP' | 'DOWN' | 'LEFT' | 'RIGHT') => {
    ...
    if (moved) {
      ...
      try {
        ...
        const newBoard = [...nextTiles, newTile];
        setTiles(newBoard);
  ...
  const checkGameOver = (currentTiles: Tile[]) => {
    if (currentTiles.length < 16) return false; // Board not full

    // Check for possible merges
    // 1. Check horizontal merges
    for (let r = 0; r < 4; r++) {
      const row = currentTiles.filter(t => t.r === r).sort((a, b) => a.c - b.c);
      for (let i = 0; i < row.length - 1; i++) {
        if (row[i].val === row[i + 1].val) return false;
      }
    }
    // 2. Check vertical merges
    for (let c = 0; c < 4; c++) {
      const col = currentTiles.filter(t => t.c === c).sort((a, b) => a.r - b.r);
      for (let i = 0; i < col.length - 1; i++) {
        if (col[i].val === col[i + 1].val) return false;
      }
    }
    return true;
  };
