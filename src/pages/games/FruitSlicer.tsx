...
export default function FruitSlicer() {
  ...
  const [countdown, setCountdown] = useState(-1); // 新增：倒计时状态，初始为-1表示未开始
  ...
  useEffect(() => {
    ...
    } else if (countdown === 0 && !gameOver && !inRound) {
      // 倒计时结束，开始回合
      startRound()
    }
  }, [countdown, gameOver, inRound]);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => setRoundTimeLeft(s => s - 1), 1000);
    return () => clearTimeout(t);
  }, [inRound, roundTimeLeft]);

  const startRound = () => {
    const ROUND_DURATION = 20; // 每回合时长（秒）
    setInRound(true);
    setIsPlaying(true);
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      // Spawn Logic： 只在回合中生成水果，并降低生成频率
      if (inRound && Date.now() - state.current.lastSpawn > 1400) {
        spawnFruit();
      ...
      <div className="flex flex-col items-center py-6 min-h-[calc(100vh-100px)] select-none">
        ...
        <div className="relative bg-slate-900 rounded-2xl overflow-hidden shadow-2xl border-8 border-slate-800 max-w-3xl w-full aspect-[4/3] cursor-none">
          ...
          <div className="absolute top-6 left-6 pointer-events-none">
            <div className="text-slate-400 text-sm font-bold uppercase tracking-wider">{t('common.score')}</div>
          ...
          <div className="absolute top-6 right-6 flex gap-2 pointer-events-none">
            {Array(MAX_LIVES).fill(0).map((_, i) => (
              <div
                key={i}
                className={`w-8 h-8 rounded-full border-2 border-slate-800 transition-colors duration-300 ${i < lives ? 'bg-red-500 shadow-[0_0_10px_rgba(239,68,68,0.6)]' : 'bg-slate-700'
                  }`}
              />
            ))}
          </div>
          ...
          <div className="absolute top-6 left-1/2 -translate-x-1/2 pointer-events-none text-center">
            <div className="text-slate-400 text-sm font-bold uppercase tracking-wider">{t('common.round', { defaultValue: 'Round' })}</div>
            <div className="text-white font-black text-2xl">{round} {inRound && <span className="text-xs text-slate-300 ml-2">- {roundTimeLeft}s</span>}</div>
          </div>
            ...
            {countdown > 0 && (
              <div className="absolute inset-0 flex items-center justify-center bg-black/40 backdrop-blur-[2px] z-20 pointer-events-none">
                <motion.div
                  ...
                  className={`text-9xl font-black italic drop-shadow-[0_10px_10px_rgba(0,0,0,0.5)] ${countdown === 1 ? 'text-red-500' :
          ...
          {(!isPlaying && (countdown < 0 || gameOver)) && (
            <div className="absolute inset-0 bg-black/70 flex flex-col items-center justify-center text-white backdrop-blur-sm z-30">
              {gameOver ? (
                ...
                >
                  ...
                  <h2 className="text-5xl font-black mb-2 italic">{t('common.game_over')}</h2>
                  <p className="text-2xl mb-8 text-slate-300">{t('common.final_score')}: <span className="text-white font-bold">{score}</span></p>
                  <button
                    onClick={prepareGame}
                  ...
                  >
                    <RefreshCw size={24} /> {t('common.try_again')}
                  </button>
                </motion.div>
              ) : (
                <motion.button
                ...
                >
                  <span className="relative z-10 flex items-center gap-3"><Play fill="currentColor" /> {t('common.start_game')}</span>
                  {/* Shiny effect */}
        ...
        <p className="mt-6 text-slate-400 flex items-center gap-2">
          <span className="bg-slate-200 text-slate-600 px-2 py-1 rounded text-xs font-bold">{t('common.tip', { defaultValue: 'TIP' })}</span>
          {t('games.fruit-ninja-tip', { defaultValue: 'Move mouse quickly to slice! Bounce fruits off walls!' })}
        </p>
      </div>
    </PageTransition>
  );
